...
#include <fstream>
#include "text_rec.h"
TextRecognizer::TextRecognizer(const std::string& model_path, const std::string& dict)
{
    ...
    for (int i = 0; i < numInputNodes; i++)
    {
        ...
        input_names.push_back(name.release());
    ...
    this->alphabet.emplace_back(" ");
    names_len = this->alphabet.size();
}

cv::Mat TextRecognizer::preprocess(cv::Mat srcimg)
{
    cv::Mat dstimg;
    int h = srcimg.rows;
    int w = srcimg.cols;
