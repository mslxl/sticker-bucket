//! 单行文本输入框：macOS text field 外观，支持 IME、选区、剪贴板。
...
/// 单行文本输入框实体。`cx.new(|cx| TextInput::new("占位符", cx))` 创建，
/// 直接作为 child 渲染。
...
impl TextInput {
    ...
    /// 无镶边变体：去掉白底与描边，用于嵌入搜索框等复合控件。
    pub fn bare(placeholder: impl Into<SharedString>, cx: &mut Context<Self>) -> Self {
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), to_hsla(theme(cx).label_3))
        } else {
            (content, style.color)
        };
        let run = TextRun {
        ...
        let mut selection_color = to_hsla(accent);
        selection_color.a = 0.2;
...
impl Render for TextInput {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let t = theme(cx);
        let mut focus_ring = to_hsla(t.accent);
        focus_ring.a = 0.5;
        div()
            ...
            .text_size(px(13.))
            .line_height(px(18.))
            .when(!self.bare, |style| {
                style
                    .border_1()
                    .border_color(t.edge)
                    .bg(t.control_bg)
                    .focus(move |style| style.border_color(focus_ring))
            })
