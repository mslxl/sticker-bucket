...
import DictText from './dict.txt?raw'
import OrtModelDet from './ppocr_det.onnx?url'
import OrtModelRec from './ppocr_rec.onnx?url'
...
export async function recognize(imageUrl) {
  // init
  det = await ort.InferenceSession.create(OrtModelDet)
  rec = await ort.InferenceSession.create(OrtModelRec)
  dic = DictText.split(/\r\n|\r|\n/)
  // get img
  const img = document.createElement('img')
  img.src = imageUrl
  const canvas = document.createElement('canvas')
  let isLoad = await new Promise((resolve, reject) => {
    (img.onload = () => {
